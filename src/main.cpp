...
// Copyright (c) 2013 fastcoinsha developers
...
const string strMessageMagic = "fastcoinsha Signed Message:\n";
...
static const int64 nTargetTimespan = 30; // 1 minutes
static const int64 nTargetSpacing = 300; // 30 seconds
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 2 blocks

static const int64 nAveragingInterval = nInterval * 200; // 40 blocks
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; // 40 minutes

static const int64 nMaxAdjustDown = 150; // 20% adjustment down
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("fastcoinshaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("fastcoinshaMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("fastcoinshaMiner started\n");
    ...
    try { loop {
        ...
        printf("Running fastcoinshaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("fastcoinshaMiner terminated\n");
        throw;
    }
}
